...
export default async function fetchJsonLd(url: string, options: RequestInitExtended = {}): Promise<ResponseDocument | EmptyResponseDocument> {
  ...
  if (status >= 500 || !contentType || (!contentType.includes(jsonLdMimeType) && !contentType.includes(jsonProblemMimeType))) {
...
function setHeaders(options: RequestInitExtended): RequestInit {
  ...
  if (result.body !== 'undefined' && !(typeof FormData !== 'undefined' && result.body instanceof FormData) && result.headers.get('Content-Type') === null) {
