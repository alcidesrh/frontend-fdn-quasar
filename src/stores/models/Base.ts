...
export default function (name) {
  ...
  const state = {
    ...
    item: {},
    options: [],
    fields: useCloned(entity.fields).cloned.value,
  ...
  return {
    ...
    actions: {
      ...
      async getFormSchema() {
        ...
        for (const field of this.fields) {
          ...
          const label = this.labels.find((v) => field.name in v);
          ...
          formSchema.push({
            $formkit: fkType,
            name: field.name,
            label: label ? label[field.name] : field.name,
        ...
        this.formData = await this.getFormData(this.fields);
      },
      async getFormData() {
        let storeName = "";
        for (const field of this.fields) {
          ...
          if (fieldType.kind === "OBJECT" || fieldType.kind === "LIST") {
            if (fieldType.kind === "LIST") {
              ...
              if (fieldType.name.endsWith("PageConnection")) {
                storeName = fieldType.name.substring(
                  0,
                  fieldType.name.length - 14,
                );
              } else {
      ...
      submit(data) {
        const temp = {};
        temp[this.nameDecapitalize] = this.getQueryFields();
        const query = queryBuilder.mutation({
    ...
    getters: {
      name: (s) => s.name,
      nameDecapitalize: (s) => str.decapitalize(s.name),
      ...
      iri: (s) => `/api/${s.nameDecapitalize}s/${s.item.id}`,
      getQueryFields: (s) =>
        entities.value[s.name].fields
          .filter((v) => !s.excludeFields.includes(v.name))
          .map((v) => {
